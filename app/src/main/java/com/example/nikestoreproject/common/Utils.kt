...
fun convertPxToDp(dpValue: Float, context: Context?): Float {
    return if (context != null) {
        val resources = context.resources
        val metrics = resources.displayMetrics
        dpValue * (metrics.densityDpi.toFloat() / DisplayMetrics.DENSITY_DEFAULT)
    } else {
...
fun formatPrice(price: Number, unitRelativeSizeFactor: Float = 0.65f): SpannableString {
    val label = "تومان"
    val spannableString = SpannableString("$price $label")
